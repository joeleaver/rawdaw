...
/// the M5 default. The test installs a patch with the filter cutoff
/// pushed up to 8 kHz (vs the default 800 Hz) and verifies the
/// rendered audio diverges. This is the load-bearing pin that
/// `WavetableSynthNode::with_patch` is wired — without it, the
/// node would silently fall back to the default.
#[test]
