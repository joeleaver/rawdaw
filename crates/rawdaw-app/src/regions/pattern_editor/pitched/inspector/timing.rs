//! Timing fields (time + duration) for the per-note inspector.
//!
//! Each input mirrors the canonical event field via a one-shot
//! buffer-sync `Effect` at mount (reads are `untracked` so subsequent
//! project bumps don't re-enter the Effect's RefCell during cascading
//! flushes — see the inspector shell doc-comment).

use rinch::core::reactive::{untracked, Effect};
use rinch::prelude::*;

use rawdaw_model::id::{NoteId, PatternId};
use rawdaw_model::pattern::{PatternBody, PitchedPatternBody};
